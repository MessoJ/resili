...
1. **TODO 1:** In `apps/console/public/sw.js`, ensure the offline fallback message returns proper JSON if both cache and network fail during a risk fetch.
...
3. **TODO 3:** In `apps/console/src/app/layout.tsx`, verify that the service worker is registered on window load:
   ```ts
